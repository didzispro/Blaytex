...
public class PlayerController2 : MonoBehaviour
{
    ...
    void AttackStuff()
    {
        if (Input.GetKeyDown(KeyCode.F) && gameManager2.canAttack)
        {
            Attack();
        }

        if (Input.GetKeyDown(KeyCode.G) && gameManager2.canAttack)
        {
            AttackHorizontal();
        }
    }
    ...
    void RollMovement()
    {  
        if (Input.GetKey(KeyCode.A) && Input.GetKeyDown(KeyCode.S) && gameManager2.canAttack && isRolling1 && gameManager2.canStart)
        {
            AnimationStartsLeft();
        }

        if (Input.GetKey(KeyCode.D) && Input.GetKeyDown(KeyCode.S) && gameManager2.canAttack && isRolling1 && gameManager2.canStart)
        {
    ...
    void Attack()
    {
        ...
        // Detect player2 in range of attack.
        Collider2D[] hitplayer2 = Physics2D.OverlapCircleAll(attackPoint.position, attackRange, enemyLayer);

        // Damage them.
        foreach (Collider2D player2 in hitplayer2)
        {
            player2.GetComponent<Multiplayer>().TakeDamage(attackDamage);

            float dir = transform.localScale.x;

            player2.GetComponent<Multiplayer>().Knockback(dir);
        }
    ...
    void Die()
    {
        ...
        gameManager2.OnRoundEnd(GameManager2.RoundResult.Player);
